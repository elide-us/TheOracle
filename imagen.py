import asyncio
from imagen_base import DISCORD, DISCORD_TOKEN, DISCORD_CHANNEL
from imagen_openai import a_generate_audio, a_generate_text, co_run_images, simple_parser
from imagen_lumaai import a_generate_video
...
async def a_handle_image_generate(args: str, channel: str):
  if len(args) < 2:
    raise ValueError("Image generate requires at least 2 arguments.")
  
  template, palette = args

  #arguments_str = " ".join(args[1:])
  #arguments = simple_parser(arguments_str)
  
  #return await co_run_images(template, arguments, channel)
  return await co_run_images(template, palette, channel)

async def a_handle_video_generate(args: str, channel: str):
  if len(args) != 3:
    if channel:
      await channel.send("Video generate requires 3 arguments: prompt, start_asset, end_asset")
  prompt, start_asset, end_asset = args
  asyncio.create_task(a_generate_video(prompt, start_asset, end_asset, channel))
